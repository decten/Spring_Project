...
-> split해서 쿼리문(/calculate?~operand2=55)만 가져와서 처리한다
- 요청 클래스 설계
   - HttpReqeust(요청): RequestLine,Header(리퀘스트 라인만 사용하면 돼서 얘도 별 쓸모 없음), Body(요청 메시지는 헤더와 바디 사이에 blank line 있지만 공백이라 생략)
3. LoggerFactory
  - gradle 오류 
```java
